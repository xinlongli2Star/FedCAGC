...
def parse_args():
    parser = argparse.ArgumentParser(
        description="FedCAGC V2 mechanism-experiment runner."
    )
    ...
    parser.add_argument("--method", type=str, default="fedcagc",
                        choices=["fedcagc", "pcgrad_history"])
    ...
    parser.add_argument("--surgery_scope", type=str, default="final_classifier",
                        ...
                        help="Final classifier or broader last-two-FC scope for the correction-space comparison.")
    ...
    parser.add_argument("--prototype_mode", type=str, default="ema", choices=["ema", "fresh_round_start"],
                        help="ema uses historical prototypes; fresh_round_start uses same-round current-gradient references.")
    ...
    if args.surgery_scope == "last_two_fc" and args.dataset not in {"cifar10", "cifar100"}:
        parser.error("--surgery_scope last_two_fc is only defined for the AlexNet-style CIFAR-10 model.")
...
def is_surgery_param_name(name: str, args) -> bool:
    ...
    if args.surgery_scope == "last_two_fc":
        # Reference experiment: CIFAR-10 AlexNet classifier.4 + classifier.6.
...
def apply_pcgrad_history_correction(client_id: int, g_part: torch.Tensor,
                                     prototypes: List[Optional[torch.Tensor]], args,
                                     prototype_bank: Optional[torch.Tensor] = None):
    """Reference baseline: PCGrad-style random-order projection against historical references.
...
def compute_round_start_references(model: nn.Module, wm_train_sets: List[Dataset], args):
    ...
    This setting is used as a same-round current-gradient reference. It is not used
...
def main():
    ...
    references = {
        ...
        "fedcagc": "FedCAGC",
        "pcgrad_history": "Reference experiment: PCGrad-style historical-prototype adaptation; original PCGrad is Yu et al., NeurIPS 2020",
    }
    config = dict(vars(args))
    config.update({
        "script": "fedcagc_v2_mechanism_experiments.py",
        ...
        "fedcagc_constraints": {
            ...
            "experiment_scope": "fresh_round_start and last_two_fc are reference experiments, not the deployable default",
        },
    })
    ...
    for rnd in range(1, args.rounds + 1):
        ...
        if is_cagc_method(args.method) and args.prototype_mode == "fresh_round_start":
            ...
            if rnd == 1:
                print("Reference experiment cold-start: fresh same-round references computed, correction still starts at round 2.")
            elif rnd == 2:
        ...
        if should_eval:
            ...
            if args.method == "fedavg":
                # Measure the watermark response only for watermarked methods.
