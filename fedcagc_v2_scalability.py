...
def parse_args():
    ...
    parser.add_argument("--watermark_source", type=str, default="waffle", choices=["waffle"])
    ...
    # Sun et al. 2026 FDWA/FedDWA black-box branch.
    parser.add_argument("--fdwa_alpha", type=float, default=0.005,
                        help="Watermark-loss weight used by the cited paper.")
    parser.add_argument("--fdwa_pbase", type=float, default=-1.0,
                        help="Base aggregation weight; <0 uses the paper default 1/(2K).")
    parser.add_argument("--fdwa_id_bits", type=int, default=16,
                        help="Client identity bit length used to construct grid triggers.")
    parser.add_argument("--fdwa_noise_std", type=float, default=0.25,
                        help="Std of additive grid noise; paper leaves the magnitude unspecified.")
    ...
    # Scalability protocol constraints:
    # CIFAR-10, WafflePattern watermark, output dimension equals client count.
    if args.dataset != "cifar10":
        parser.error("Scalability experiment is fixed to CIFAR-10.")
    ...
    if args.fdwa_alpha <= 0:
        parser.error("--fdwa_alpha must be positive.")
    if args.fdwa_id_bits <= 0:
        parser.error("--fdwa_id_bits must be positive.")
    side = int(round(args.fdwa_id_bits ** 0.5))
    if side * side != args.fdwa_id_bits:
        parser.error("--fdwa_id_bits must be a perfect square (default 16 => 4x4 grid).")
    if args.fdwa_noise_std < 0:
        parser.error("--fdwa_noise_std must be non-negative.")
    ...
    if args.fdwa_pbase < 0:
        args.fdwa_pbase = 1.0 / (2.0 * args.num_clients)
    if not 0.0 <= args.fdwa_pbase <= 1.0 / args.num_clients:
        parser.error("--fdwa_pbase must be in [0, 1/K].")
...
class IdentityGridWatermarkDataset(Dataset):
    """Sun et al. 2026 identity-based black-box trigger set.

    The cited method hashes each client identity into l bits, divides an image into l grids,
    adds noise to grids whose identity bit is 1, and sets the target class to sum(bits) mod C.
    The exact noise magnitude/distribution is not fixed in the paper; V2 uses deterministic
    zero-mean Gaussian noise with configurable std and records that implementation choice.
    """
    def __init__(self, base_dataset: Dataset, indices: List[int], client_id: int,
                 identity_bits: List[int], target_label: int, spec: DatasetSpec,
                 seed: int, noise_std: float):
        self.base_dataset = base_dataset
        self.indices = [int(x) for x in indices]
        self.client_id = int(client_id)
        self.identity_bits = [int(b) for b in identity_bits]
        self.target_label = int(target_label)
        self.spec = spec
        self.seed = int(seed)
        self.noise_std = float(noise_std)
        side = int(round(len(self.identity_bits) ** 0.5))
        if side * side != len(self.identity_bits):
            raise ValueError("Identity bit length must be a perfect square.")
        self.grid_side = side
        self.mean = torch.tensor(spec.mean, dtype=torch.float32).view(spec.channels, 1, 1)
        self.std = torch.tensor(spec.std, dtype=torch.float32).view(spec.channels, 1, 1)
    ...
    def __getitem__(self, item):
        ...
        x = x.clone().float()
        h, w = x.shape[-2:]
        gh, gw = h // self.grid_side, w // self.grid_side
        gen = torch.Generator()
        gen.manual_seed(self.seed + self.client_id * 1000003 + base_index * 97)
        for bit_idx, bit in enumerate(self.identity_bits):
            if bit == 0:
                continue
            r, c = divmod(bit_idx, self.grid_side)
            r0 = r * gh
            c0 = c * gw
            r1 = h if r == self.grid_side - 1 else (r + 1) * gh
            c1 = w if c == self.grid_side - 1 else (c + 1) * gw
            patch = x[:, r0:r1, c0:c1]
            if self.noise_std > 0:
                noise = torch.randn(patch.shape, generator=gen, dtype=patch.dtype) * self.noise_std
                x[:, r0:r1, c0:c1] = torch.clamp(patch + noise, 0.0, 1.0)
        x = (x - self.mean) / self.std
        return x, self.target_label


def _identity_bits(client_id: int, num_bits: int, seed: int) -> List[int]:
    # The paper requires a hash H over client identity but does not mandate a hash function.
    # SHA-256 is used as a deterministic cryptographic instantiation of H.
    payload = f"client-{client_id}".encode("utf-8")
    ...
    for byte in digest:
        for shift in range(7, -1, -1):
            ...
            if len(bits) == num_bits:
                return bits
    raise RuntimeError("Unable to derive enough identity bits from SHA-256.")


def get_fdwa_raw_task_datasets(args):
    """Official task train/test splits without normalization/augmentation for trigger construction."""
    to_tensor = transforms.ToTensor()
    if args.dataset == "fmnist":
        train_ds = datasets.FashionMNIST(args.data_path, train=True, download=True, transform=to_tensor)
        test_ds = datasets.FashionMNIST(args.data_path, train=False, download=True, transform=to_tensor)
    elif args.dataset == "cifar10":
        train_ds = datasets.CIFAR10(args.data_path, train=True, download=True, transform=to_tensor)
        test_ds = datasets.CIFAR10(args.data_path, train=False, download=True, transform=to_tensor)
    elif args.dataset == "cifar100":
        train_ds = datasets.CIFAR100(args.data_path, train=True, download=True, transform=to_tensor)
        test_ds = datasets.CIFAR100(args.data_path, train=False, download=True, transform=to_tensor)
    else:
        raise ValueError(args.dataset)
    return train_ds, test_ds


def build_fdwa_watermark_datasets(args):
    """Build Sun2026-FDWA identity-trigger datasets from official task train/test splits."""
    spec = DATASET_SPECS[args.dataset]
    train_base, test_base = get_fdwa_raw_task_datasets(args)
    need_train = args.num_clients * args.wm_train_size
    need_test = args.num_clients * args.wm_test_size
    if need_train > len(train_base) or need_test > len(test_base):
        raise RuntimeError("FDWA watermark sample request exceeds the official dataset split size.")

    rng_train = np.random.default_rng(args.seed + 710001)
    rng_test = np.random.default_rng(args.seed + 910001)
    train_pool = np.arange(len(train_base)); rng_train.shuffle(train_pool)
    test_pool = np.arange(len(test_base)); rng_test.shuffle(test_pool)

    wm_train_sets, wm_test_sets, target_labels = [], [], []
    metadata = {
        "source": args.dataset,
        "method": "Sun2026-FDWA-blackbox",
        "split_protocol": "official_task_train100_test200_identity_grid_trigger",
        "train_source_split": f"{args.dataset} official train",
        "test_source_split": f"{args.dataset} official test",
        "identity_hash": "SHA-256",
        "identity_bits": args.fdwa_id_bits,
        "noise_distribution": "deterministic Gaussian additive noise, clipped to [0,1]",
        "noise_std": args.fdwa_noise_std,
        "noise_magnitude_note": "implementation choice because the cited paper does not specify a unique magnitude",
        "clients": {},
    }
    for cid in range(args.num_clients):
        tr0 = cid * args.wm_train_size
        te0 = cid * args.wm_test_size
        train_indices = train_pool[tr0:tr0 + args.wm_train_size].tolist()
        test_indices = test_pool[te0:te0 + args.wm_test_size].tolist()
        bits = _identity_bits(cid, args.fdwa_id_bits, args.seed)
        target = int(sum(bits) % spec.num_classes)
        wm_train_sets.append(IdentityGridWatermarkDataset(
            train_base, train_indices, cid, bits, target, spec, args.seed + 31, args.fdwa_noise_std
        ))
        wm_test_sets.append(IdentityGridWatermarkDataset(
            test_base, test_indices, cid, bits, target, spec, args.seed + 53, args.fdwa_noise_std
        ))
    ...
    # Keep the same return signature as the common builder.
    return wm_train_sets, wm_test_sets, wm_test_sets, target_labels, metadata
...
def aggregate_with_weights(client_states: List[Dict[str, torch.Tensor]], weights: Sequence[float]):
    weights = np.asarray(weights, dtype=np.float64)
    if len(weights) != len(client_states):
        raise ValueError("weights and client_states must have equal length")
    if np.any(weights < 0) or float(weights.sum()) <= 0:
        raise ValueError("aggregation weights must be non-negative and sum to >0")
...
def fdwa_weights_from_previous_trigger_rates(previous_ta: Sequence[float], args) -> np.ndarray:
    """Sun et al. Eq.(18): P_i=P_base+(1-TA_i)*(1-KP_base)/sum_j(1-TA_j)."""
    ta = np.clip(np.asarray(previous_ta, dtype=np.float64), 0.0, 1.0)
    residual = 1.0 - ta
    budget = 1.0 - args.num_clients * args.fdwa_pbase
    denom = float(residual.sum())
    if denom <= args.eps or budget <= args.eps:
        return np.ones(args.num_clients, dtype=np.float64) / args.num_clients
    weights = args.fdwa_pbase + residual * budget / denom
...
def save_history_csv(history: List[Dict], args):
    ...
    fieldnames = [
        ...
        "fdwa_alpha", "fdwa_pbase", "fdwa_id_bits", "fdwa_noise_std", "seed",
    ]
    ...
    with open(args.csv_path, "w", newline="", encoding="utf-8") as f:
        ...
        for record in history:
            ...
            row.update({
                ...
                "awm_ema": args.awm_ema, "fdwa_alpha": args.fdwa_alpha,
                "fdwa_pbase": args.fdwa_pbase, "fdwa_id_bits": args.fdwa_id_bits,
                "fdwa_noise_std": args.fdwa_noise_std, "seed": args.seed,
...
def main():
    ...
    config.update({
        "script": "fedcagc_v2_scalability.py",
    ...
    if args.method == "fdwa":
        print(f"{'fdwa_alpha':24s}: {args.fdwa_alpha}")
        print(f"{'fdwa_pbase':24s}: {args.fdwa_pbase}")
        print(f"{'fdwa_id_bits':24s}: {args.fdwa_id_bits}")
        print(f"{'fdwa_noise_std':24s}: {args.fdwa_noise_std}")
    print(f"{'output_dir':24s}: {output_dir}")

    train_dataset, test_dataset = get_main_datasets(args)
    if args.method == "fdwa":
        wm_train_sets, wm_extra_sets, wm_test_sets, target_labels, wm_metadata = build_fdwa_watermark_datasets(args)
    else:
    ...
    for i in range(args.num_clients):
        if args.method == "fdwa":
            source_desc = f"{args.dataset} identity-grid trigger"
        else:
    ...
    fdwa_previous_ta = np.zeros(args.num_clients, dtype=np.float64)
    ...
    for rnd in range(1, args.rounds + 1):
        ...
        if args.method == "fedawm":
            ...
            allocation = (np.ones(args.num_clients) / args.num_clients if rnd == 1
                          else fdwa_weights_from_previous_trigger_rates(fdwa_previous_ta, args))
            print(f"FDWA aggregation weights mean/min/max: {allocation.mean():.4f}/{allocation.min():.4f}/{allocation.max():.4f}")
        else:
        ...
        if args.method == "fdwa":
            global_model.load_state_dict(aggregate_with_weights(client_states, allocation))
        else:
        ...
        # FDWA Eq.(18) uses the previous round's per-client trigger accuracy.
        # Use the WATERMARK-TRAIN set only so the official test set never drives training/aggregation.
        if args.method == "fdwa":
            fdwa_previous_ta, _, _, _, _ = evaluate_watermarks(global_model, wm_train_sets, target_labels, args)
            print("FDWA train-trigger TA:", " ".join(f"{x:.3f}" for x in fdwa_previous_ta))
        ...
        if should_eval:
            ...
            # FDWA target labels may repeat by construction; an off-diagonal target-label matrix is not an identity FAR.
            reported_cross = float("nan") if args.method == "fdwa" else cross_wma
            record = {
    ...
    run_summary = {
        ...
        "fdwa_offdiagonal_note": "NaN because multiple identity triggers may legitimately share the same target label" if args.method == "fdwa" else "not applicable",
    }
