...
Methods included:
  ...
  fedcagc  : conflict-aware gradient correction proposed in the manuscript.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="FedCAGC V2 reviewer-experiment runner (supplemental diagnostics only)."
    )
    ...
    parser.add_argument("--method", type=str, default="fedcagc",
                        choices=["fedavg", "fedipr", "flwb", "fedawm", "fedcagc", "pcgrad_history"])
    ...
    parser.add_argument("--surgery_scope", type=str, default="final_classifier",
                        choices=["final_classifier", "last_two_fc"],
                        help="Reviewer diagnostic only: default final classifier or broader last-two-FC scope on CIFAR-10 AlexNet.")
    parser.add_argument("--ema_rho", type=float, default=0.9)
    parser.add_argument("--prototype_mode", type=str, default="ema", choices=["ema", "fresh_round_start"],
                        help="ema is deployable FedCAGC; fresh_round_start is an oracle-like same-round reference diagnostic.")
    ...
    if args.surgery_scope == "last_two_fc" and args.dataset not in {"cifar10", "cifar100"}:
        parser.error("--surgery_scope last_two_fc is only defined for the AlexNet-style CIFAR model in this reviewer runner.")
...
def is_surgery_param_name(name: str, args) -> bool:
    if args.surgery_scope == "final_classifier":
        return name.startswith("fc2.") or name.startswith("classifier.6.")
    if args.surgery_scope == "last_two_fc":
        # Reviewer diagnostic: CIFAR-10 AlexNet classifier.4 + classifier.6.
        # For the FMNIST CNN there is only one explicit final FC block in this implementation,
        # so this scope is intentionally restricted to CIFAR-like AlexNet experiments.
        return name.startswith("classifier.4.") or name.startswith("classifier.6.")
    return False
...
def is_cagc_method(method: str) -> bool:
    return method in {"fedcagc", "pcgrad_history"}
...
def apply_pcgrad_history_correction(client_id: int, g_part: torch.Tensor,
                                     prototypes: List[Optional[torch.Tensor]], args,
                                     prototype_bank: Optional[torch.Tensor] = None):
    """Reviewer baseline: PCGrad-style random-order projection against historical references.

    This is an adaptation baseline, not the original multi-task PCGrad setting. It uses the
    same historical prototype information and final-layer communication budget as FedCAGC,
    but removes every currently negative projection in a deterministic pseudo-random order
    and does not apply norm compensation or conflict-strength sorting.
    """
    original = g_part
    g = g_part.clone()
    bank = prototype_bank
    ...
    rng = np.random.default_rng(args.seed + 7919 * (client_id + 1))
    order_np = rng.permutation(bank.shape[0])
    order = torch.as_tensor(order_np, device=g.device, dtype=torch.long)
    for pos in range(bank.shape[0]):
        h = bank[order[pos]]
        dot_gh = torch.dot(g, h)
        denom_cos = torch.norm(g, p=2) * torch.norm(h, p=2) + args.eps
        negative = (dot_gh / denom_cos) < 0.0
        projected = g - (dot_gh / (torch.dot(h, h) + args.eps)) * h
        g = torch.where(negative, projected, g)

    n0 = torch.norm(original, p=2)
    n1 = torch.norm(g, p=2)
    retain = n1 / (n0 + args.eps)
    return g, retain.detach(), retain.detach()


def compute_round_start_references(model: nn.Module, wm_train_sets: List[Dataset], args):
    """Compute same-round raw watermark-gradient references on the round-start global model.

    This is intentionally an oracle-like diagnostic requested by the reviewer. It is not used
    as the deployable FedCAGC protocol and requires access to every client's current watermark
    gradient before local training.
    """
    ...
    for wm_set in wm_train_sets:
        loader = DataLoader(wm_set, batch_size=len(wm_set), shuffle=False, num_workers=0,
        ...
        g = flatten_grads(grads, params).detach()
        refs.append(normalize_or_none(g, args.eps))
    if was_training:
        model.train()
    return refs
    ...
    for _ in range(args.local_epochs):
        ...
        for x_task, y_task in task_loader:
            ...
            if is_cagc_method(args.method) and use_surgery:
                if args.method == "pcgrad_history":
                    fixed_part, r_before, r_after = apply_pcgrad_history_correction(
                        client_id, raw_part, prototypes, args, prototype_bank=prototype_bank
                    )
                else:
...
def evaluate_global_gradient_conflicts(model: nn.Module, wm_train_sets: List[Dataset],
    ...
    if use_correction:
        if args.method == "pcgrad_history":
            corrected_grads = [
                apply_pcgrad_history_correction(client_id, grad, prototypes, args)[0].detach()
...
def main():
    ...
    references = {
        ...
        "fedcagc": "Proposed method in the current manuscript",
        "pcgrad_history": "Reviewer diagnostic: PCGrad-style historical-prototype adaptation; original PCGrad is Yu et al., NeurIPS 2020",
    }
    config = dict(vars(args))
    config.update({
        "script": "fedcagc_v2_review_experiments.py",
        ...
        "comparison_protocol": {
            "fedipr_flwb_fedawm_fedcagc_watermark": "same official MNIST train100/test200 trigger sets and target labels, per submitted manuscript fairness protocol",
        ...
        "fedcagc_constraints": {
            ...
            "review_scope_note": "fresh_round_start and last_two_fc are reviewer diagnostics, not the deployable default",
        },
    })
    ...
    if is_cagc_method(args.method):
        print(f"{'surgery_scope':24s}: {args.surgery_scope}")
        print(f"{'prototype_mode':24s}: {args.prototype_mode}")
    if args.method == "flwb":
    ...
    for rnd in range(1, args.rounds + 1):
        ...
        if is_cagc_method(args.method) and args.prototype_mode == "fresh_round_start":
            round_prototypes = compute_round_start_references(global_model, wm_train_sets, args)
            # Keep the same cold-start schedule as deployable FedCAGC so that the only
            # changed factor is reference freshness, not the number of corrected rounds.
            use_surgery = rnd >= 2 and all(p is not None for p in round_prototypes)
            if rnd == 1:
                print("Reviewer diagnostic cold-start: fresh same-round references computed, correction still starts at round 2.")
            elif rnd == 2:
                print(f"Fresh-reference correction starts: {use_surgery}")
        else:
            ...
            if is_cagc_method(args.method) and rnd == 1:
                print("Cold-start round: no correction; collect raw watermark gradients for historical prototypes.")
            elif is_cagc_method(args.method) and rnd == 2:
                print(f"Gradient correction starts: {use_surgery}")
        ...
        if is_cagc_method(args.method) and args.prototype_mode == "ema":
        ...
        if rnd in args.snapshot_rounds:
            prototype_snapshots[rnd] = cpu_prototypes(round_prototypes if args.prototype_mode == "fresh_round_start" else prototypes)
    ...
    saved_prototypes = round_prototypes if (is_cagc_method(args.method) and args.prototype_mode == "fresh_round_start") else prototypes
    torch.save({"model_state_dict": checkpoint_state, "prototypes": cpu_prototypes(saved_prototypes),
