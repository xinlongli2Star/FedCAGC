...
def cmd_ownership(args):
    ...
    out.to_csv(outdir / 'ownership_cross_identity.csv', index=False)
    if not out.empty:
        agg = out.groupby('dataset').agg(['mean', 'std'])
        agg.to_csv(outdir / 'ownership_cross_identity_summary.csv')
    print(out.to_string(index=False))
    print(f'SAVED {outdir / "ownership_cross_identity.csv"}')
...
def cmd_snapshot(args):
    ...
    for rnd in common:
        ...
        # this is an order-sensitivity analysis on fixed saved snapshots, not a re-run of training.
        variants = {}
        retain = {}
...
def cmd_leakage(args):
    ...
    outdir=root/'results'/'analysis'; outdir.mkdir(parents=True,exist_ok=True)
    cdf=pd.DataFrame(client_rows)
    if not cdf.empty:
        cdf[['client','best_balanced_accuracy']].to_csv(
            outdir/f'{args.dataset}_seed{args.seed}_prototype_leakage_client_ba.csv', index=False
        )
    if all_rows:
        y=np.array([r['member'] for r in all_rows]); sc=np.array([r['score'] for r in all_rows])
        ba,thr=best_bal_acc(y,sc)
        overall={'dataset':args.dataset,'seed':args.seed,'best_balanced_accuracy':ba}
        with (outdir/f'{args.dataset}_seed{args.seed}_prototype_leakage_ba.json').open('w') as f:
...
def cmd_stats(args):
    ...
    for ds in args.datasets:
        for method in args.methods:
            for seed in args.seeds:
                if method == 'tramark':
                    run=root/'results'/'experiments'/'tramark'/f'{ds}_{seed}'
                else:
                    run=root/'results'/'main'/f'{ds}_{method}_{seed}'
                sp=run/'run_summary.json'
                if not sp.exists():
                    continue
                s=read_json(sp)
                if method == 'fedavg':
                    row={'dataset':ds,'method':method,'seed':seed,
                         'MTA':s.get('final_mta'),
                         'WMA':np.nan,'MinWMA':np.nan,'WGC_pre':np.nan,'WGC_post':np.nan}
                elif method == 'tramark':
                    row={'dataset':ds,'method':method,'seed':seed,
                         'MTA':s.get('final_mta'),'WMA':s.get('final_wma'),
                         'MinWMA':np.nan,'WGC_pre':np.nan,'WGC_post':np.nan}
                else:
                    row={'dataset':ds,'method':method,'seed':seed,
                         'MTA':s.get('final_mta'),'WMA':s.get('final_wma'),
                         'MinWMA':s.get('final_min_wma'),
    ...
    if not df.empty:
        for (ds,m),g in df.groupby(['dataset','method']):
            ...
            for col in ['MTA','WMA','MinWMA','WGC_pre','WGC_post']:
                ...
                if col in {'MTA','WMA','MinWMA'}:
                    low=max(0.0,low); high=min(1.0,high)
                    mean=round(mean,4); sd=round(sd,4); low=round(low,4); high=round(high,4)
                if col in {'WGC_pre','WGC_post'}:
                    mean=round(mean,4); sd=round(sd,4); low=round(low,4); high=round(high,4)
                r[col+'_mean']=mean
                r[col+'_sd']=sd
                r[col+'_ci95_low']=low
                r[col+'_ci95_high']=high

            pre=r.get('WGC_pre_mean')
            post=r.get('WGC_post_mean')
            if pre is not None and post is not None and pre > 0:
                r['WGC_reduction_percent']=round((pre-post)/pre*100.0,2)
            summaries.append(r)

    sdf=pd.DataFrame(summaries)
    sdf.to_csv(outdir/'main_mean_sd_ci95.csv',index=False)

    # T-tests against FedCAGC on metrics that are defined for both methods.
    trows=[]
    if not df.empty:
        for ds in args.datasets:
            ref=df[(df.dataset==ds)&(df.method=='fedcagc')]
            if ref.empty:
                continue
            for method in args.methods:
                if method=='fedcagc':
                    continue
                cur=df[(df.dataset==ds)&(df.method==method)]
                if cur.empty:
                    continue
                for metric in ['MTA','WMA','WGC_pre','WGC_post']:
                    a=ref[metric].dropna().to_numpy(float)
                    b=cur[metric].dropna().to_numpy(float)
                    if len(a)<2 or len(b)<2:
                        continue
                    p=float(scipy_stats.ttest_ind(a,b,equal_var=False).pvalue)
                    trows.append({'dataset':ds,'method':method,'metric':metric,'p_value':round(p,4)})
    tdf=pd.DataFrame(trows)
    tdf.to_csv(outdir/'main_ttest.csv',index=False)
...
def main():
    ...
    p=sub.add_parser('stats'); p.add_argument('--datasets',nargs='+',default=['cifar10','fmnist']); p.add_argument('--methods',nargs='+',default=['fedavg','fedipr','flwb','fedawm','tramark','fedcagc']); p.add_argument('--seeds',nargs='+',type=int,default=[3047,3048,3049]); p.set_defaults(func=cmd_stats)
