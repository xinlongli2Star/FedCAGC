...
TraMark creates a PERSONALIZED model for each client. Therefore:
  MTA = mean main-task test accuracy across the 10 personalized models;
  WMA = mean diagonal accuracy: model i on client i's independent watermark test set;
  VR  = fraction of personalized models whose highest watermark-set response identifies
        the correct client, matching TraMark's model-leakage verification definition.
WGC is not reported because TraMark does not produce a single multi-watermark global model.
"""
...
def evaluate_personalized(model_template, states, task_test_loader, wm_train_loaders, wm_test_loaders, device):
    mtas, task_losses = [], []
    n = len(states)
    test_matrix = np.zeros((n, n), dtype=np.float64)
    train_diag = np.zeros(n, dtype=np.float64)
    for cid in range(n):
        model = copy.deepcopy(model_template)
        load_state(model, states[cid], device)
        a, l = eval_loader(model, task_test_loader, device)
        mtas.append(a); task_losses.append(l)
        train_diag[cid] = eval_loader(model, wm_train_loaders[cid], device)[0]
        for wid in range(n):
            test_matrix[cid, wid] = eval_loader(model, wm_test_loaders[wid], device)[0]
        model.to("cpu")
        del model
    diag = np.diag(test_matrix)
    off = test_matrix[~np.eye(n, dtype=bool)]
    predicted_owner = np.argmax(test_matrix, axis=1)
    vr = float(np.mean(predicted_owner == np.arange(n)))
    return {
        "mta": float(np.mean(mtas)),
        "mta_min": float(np.min(mtas)),
        "task_loss": float(np.mean(task_losses)),
        "wma": float(np.mean(diag)),
        "wma_train": float(np.mean(train_diag)),
        "min_wma": float(np.min(diag)),
        "std_wma": float(np.std(diag, ddof=0)),
        "cross_wma": float(np.mean(off)) if off.size else 0.0,
        "cross_wma_max": float(np.max(off)) if off.size else 0.0,
        "vr": vr,
        "per_client_wma": diag.tolist(),
        "verification_matrix": test_matrix,
    }
...
def main():
    ...
    for rnd in range(1, args.rounds + 1):
        ...
        if should_eval(rnd, args.rounds, args.eval_every, args.eval_tail):
            ...
            ev = evaluate_personalized(
                model_template, states, task_test_loader, wm_train_loaders, wm_test_loaders, device
            )
            ...
            row = {
                ...
                "mta_min": ev["mta_min"],
                "wma": ev["wma"],
                "wma_train": ev["wma_train"],
                "min_wma": ev["min_wma"],
                "std_wma": ev["std_wma"],
                "cross_wma": ev["cross_wma"],
                "cross_wma_max": ev["cross_wma_max"],
                "vr": ev["vr"],
            ...
            print(f"WMA test/train       : {ev['wma']:.4f} / {ev['wma_train']:.4f}", flush=True)
            print(f"Min/Std WMA          : {ev['min_wma']:.4f} / {ev['std_wma']:.4f}", flush=True)
            print(f"Cross-WMA mean/max   : {ev['cross_wma']:.4f} / {ev['cross_wma_max']:.4f}", flush=True)
            print(f"TraMark VR           : {ev['vr']:.4f}", flush=True)
            print(f"Round/Eval time (s)  : {round_s:.2f} / {eval_s:.2f}", flush=True)

            if rnd == args.rounds:
                # Final verification matrix: rows=model/client, columns=claimed watermark identity.
                mat = ev["verification_matrix"]
                with (out / "verification_matrix.csv").open("w", newline="", encoding="utf-8") as f:
                    w = csv.writer(f)
                    w.writerow(["model_client"] + [f"claimed_client_{i}" for i in range(args.num_clients)])
                    for i in range(args.num_clients):
                        w.writerow([i] + [float(x) for x in mat[i]])
        else:
    ...
    summary = {
        ...
        "final_train_wma": final_eval["wma_train"],
        "final_min_wma": final_eval["min_wma"],
        "final_std_wma": final_eval["std_wma"],
        "final_cross_wma": final_eval["cross_wma"],
        "final_cross_wma_max": final_eval["cross_wma_max"],
        "final_vr": final_eval["vr"],
        "final_wma_per_client": final_eval["per_client_wma"],
        "wgc": None,
        "wgc_note": "Not directly applicable: TraMark maintains personalized watermark regions rather than one shared multi-watermark global model.",
    ...
    print(f"Final Min-WMA         : {summary['final_min_wma']:.4f}")
    print(f"Final Cross-WMA       : {summary['final_cross_wma']:.4f}")
    print(f"Final VR              : {summary['final_vr']:.4f}")
